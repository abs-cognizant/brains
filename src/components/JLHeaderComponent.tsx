...
export default function JLHeaderComponent() {
    ...
    <header className="border-b shadow-sm relative z-50">
      ...
      <div className="flex items-center justify-between p-[24px] bg-white">
        ...
        <div className="hidden md:flex mx-4 relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground w-5 h-5 pointer-events-none" color="black" style={{ scale: 1.25}}/>
          <Input
            type="text"
            placeholder="Search product or brand"
            value={search}
              ...
              <div className="absolute left-0 top-full mt-2 min-w-[300px] bg-white shadow-xl border rounded p-6 z-50">
                ...
                <div className="grid grid-cols-2 gap-4">
                  {cat.links.map((sub) => (
                    <Link
                      key={sub}
                      href={`/${cat.name.toLowerCase().replace(/ & /g, "-")}/${sub
                        .toLowerCase()
                        .replace(/ /g, "-")}`}
